...
boost::any MaterialStack::getProperty(unsigned long index, int id) const {
    if (index >= materials.size())
        throw Base::RuntimeError(str(boost::format("Index %1% out of range") % index));
...
void MaterialStack::insert(const std::string &name, int index)
{
    ...
    else if (index >= 0 && static_cast<size_t>(index) <= materials.size())
...
Material *MaterialStack::getMaterial(int index) const
{
    ...
    return getDatabase()->getMaterial(materials[static_cast<size_t>(index)].c_str());
}
...
void MaterialStack::Save(Base::Writer & writer) const
{
    ...
    while (vit != materials.end()) {
        writer.Stream() << writer.ind() << "<Material name=\"" <<  Property::encodeAttribute(*vit) <<"\"/>";
        ++vit;
    }
}
...
{
    ssize_t faceCount = reader.getAttributeAsInteger("count");

    for (ssize_t j = 0; j < faceCount; ++j) {
