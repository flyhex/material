...
boost::any MaterialDatabase::color_str2any(const std::string & data)
{
    ...
    if (boost::regex_match(data.c_str(), cm, e)) {
        ...
        throw Base::RuntimeError(str(boost::format("Invalid value: %1%") % data));
}
...
boost::any MaterialDatabase::color_python2any(const PyObject * data)
{
    ...
    if (PyArg_ParseTuple(const_cast<PyObject*>(data), "ffff", &r, &g, &b, &a))
        ...
        throw Base::RuntimeError("Failed to convert python object.");
}
...
boost::any MaterialDatabase::python2string(const PyObject * data)
{
    ...
    std::string string = PyBytes_AS_STRING(unicode);
    Py_DECREF(unicode);
...
boost::any MaterialDatabase::density2any(const std::string & data)
{
    try {
        ...
        if (value.getUnit().isEmpty())
            ...
            throw Base::RuntimeError(str(boost::format("Invalid unit for density %1%") % data));

        return std::move(value);
    }
    catch (Base::RuntimeError & e) {
        throw Base::RuntimeError(str(boost::format("Failed to parse density quantity %1%: %2%") % data % e.what()));
    }
}
...
boost::any MaterialDatabase::pressure2any(const std::string & data)
{
    try {
        ...
        if (value.getUnit().isEmpty()) {
            ...
            throw Base::RuntimeError(str(boost::format("Invalid unit for pressure: %1%") % data));

        return std::move(value);
    }
    catch (Base::RuntimeError & e) {
        throw Base::RuntimeError(str(boost::format("Failed to parse pressure quantity %1%: %2%") % data % e.what()));
    }
}
...
boost::any MaterialDatabase::fromString(const std::string &key, const std::string &value) const
{
    ...
    if (it == propertyMap.end())
        throw Base::RuntimeError(str(boost::format("Invalid property: %1%") % key));
...
std::string MaterialDatabase::toString(const std::string &key, const boost::any &value) const
{
    ...
    if (value.empty())
        throw Base::RuntimeError(str(boost::format("Property %1% is empty.") % key));
...
PyObject *MaterialDatabase::toPyObject(const char *propName, const boost::any &value) const
{
    ...
    if (it == propertyMap.end())
        throw Base::RuntimeError(str(boost::format("Invalid property: %1%") % propName));

    if (value.empty())
        throw Base::RuntimeError(str(boost::format("Property %1% not found.") % propName));
