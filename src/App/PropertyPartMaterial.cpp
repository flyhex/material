...
void PropertyPartMaterial::setPyObject(PyObject *)
{
...
MaterialDatabase *PropertyPartMaterial::getDatabase() const
{
    ...
    if (obj) {
        if (obj->getDocument() == nullptr)
            throw Base::RuntimeError("The owning object must be attached to a document.");
        else
            return &obj->getDocument()->getMaterialDatabase();
    }
    else
        throw Base::RuntimeError("PropertyPartMaterial must be owned by an App::DocumentObject");
}
