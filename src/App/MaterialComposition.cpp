...
boost::any MaterialComposition::getProperty(int id) const
{
    for (auto it = materials.rbegin(); it != materials.rend(); ++it) {
        ...
        if (!mat)
            throw Base::RuntimeError(str(boost::format("Unknown material: %1%") % *it));
...
void MaterialComposition::insert(const std::string &name, int index) {
    if (index == -1)
        ...
        throw Base::RuntimeError(str(boost::format("Inedex %1% out of range") % index));
}
    ...
    if (strcmp(name, "Surface") == 0)
        ...
        if (mc) {
            try {
                ...
                throw Py::Exception(Base::BaseExceptionFreeCADError, boost::str(boost::format("Unable to resolve property %1% (missing material?)") % name));
            }
        }
