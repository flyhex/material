    ...
    for (auto it = properties.begin(); it != properties.end(); ++it, ++i) {
        if (!(*it).empty()) {
            ...
            if (it->type() == typeid(Material::deleted_property_t))
                writer.Stream() << writer.ind() << "<Property name=\"" << Property::encodeAttribute(key) << "\" />" << std::endl;
            else
                writer.Stream() << writer.ind() << "<Property name=\"" << Property::encodeAttribute(key) << "\" value=\"" << Property::encodeAttribute(value) << "\"/>" << std::endl;
        }
    }
    writer.decInd();
...
{
    writer.Stream() << writer.ind() << "<PropertyDocumentMaterialSource name=\"" << Property::encodeAttribute(source->getName()) << "\" count=\"" << source->getNumMaterials() << "\">" << std::endl;
    writer.incInd();
